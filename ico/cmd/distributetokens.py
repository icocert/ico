...
@click.option('--allow-zero/--no-allow-zero', default=False,
              help='Stops the script if a zero amount row is encountered')
def main(chain, address, token, csv_file, limit, start_from, issuer_address,
    ...
    with project.get_chain(chain) as c:
        ...
        if not issuer_address:
            ...
            issuer, txhash = c.provider.deploy_contract("Issuer",
