...
@click.option('--address', nargs=1,
              help='Owner account (must exist on Ethereum node)', required=True)
@click.option('--contract-address', nargs=1,
              help='RebuildCrowdsale contract address', required=True)
@click.option('--csv-file', nargs=1,
              help='CSV export of existing data created with extract-raw-investment-data ',
              required=True)
...
@click.option('--multiplier', nargs=1,
              help='Token amount multiplier, to fix decimal place, as 10^exponent',
...
def main(chain, address, contract_address, csv_file, limit, start_from,
    ...
    with project.get_chain(chain) as c:
        ...
        assert relaunched_crowdsale.call().owner().lower() == address.lower(), "We are not the crowdsale owner. Real owner is {}, we are {}".format(
        ...
        for i in range(start_from, min(start_from + limit, len(rows))):
            ...
            if not tokens % 1 == 0:
                raise RuntimeError(
                    "Could not issue tokens because after multiplication was not integer: {} {} {}".format(
                        tokens, fractional_tokens, multiplier))
            ...
            print("Row", i, "giving", tokens, "to", addr, "from tx", orig_txid,
                  "ETH spent", spent, "time passed", time.time() - start_time,
            ...
            if cap_check:
                ...
                if relaunched_crowdsale.call().isBreakingCap(wei, tokens,
                                                             relaunched_crowdsale.call().weiRaised(),
                                                             relaunched_crowdsale.call().tokensSold()):
                    raise RuntimeError("Cap error")

            txid = relaunched_crowdsale.transact(
                transaction).setInvestorDataAndIssueNewToken(addr, wei, tokens,
                                                             orig_txid)
