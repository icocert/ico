...
@click.option('--amount-column', nargs=1,
              ...
              default="amount")
@click.option('--external-id-column', nargs=1,
              help='Name of CSV column containing the external id',
              default="external_id")
...
@click.option('--issuer-address', nargs=1,
              help='The address of the issuer contract - leave out for the first run to deploy a new issuer contract',
              required=False, default=None)
@click.option('--gas-price', nargs=1,
              help='Override gas price. If not set use the default * 2.0. Specify in Gwei e.g. 50.',
              required=False, default=None)
@click.option('--master-address', nargs=1,
              help='The team multisig wallet address that does StandardToken.approve() for the issuer contract',
              required=False, default=None)
@click.option('--solc-version', nargs=1,
              help='Menu item for the solc compiler verification on EtherScan',
              required=False, default="v0.4.16+commit.d7661dd9")
@click.option('--allow-addresless', default=True,
              help='If address column is not filled in skip the participant')
def main(chain, address, token, csv_file, limit, start_from, issuer_address,
    ...
    with project.get_chain(chain) as c:
        ...
        if not issuer_address:
            ...
            issuer, txhash = c.provider.deploy_contract("IssuerWithId",
        ...
        for i in range(start_from, min(start_from + limit, len(rows))):
            ...
            if not external_id:
                raise RuntimeError(
                    "Missing external id on row #{}".format(i + 1))
            ...
            print("Row", i, "giving", tokens, "to", addr, "issuer",
                  issuer.address, "time passed", time.time() - start_time,
                  "ETH passed", spent, "gas price",
